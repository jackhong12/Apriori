...
public:
    Apriori (ItemSets _transactions, long double _minSupport) {
        ...
        // move transactions data to cuda memory
        int size = transactionBit.size * transactionBit.unit_len * sizeof(Bit);
        cudaMalloc((void **)&cuda_transaction_buffer, size);
        cudaMemcpy(cuda_transaction_buffer, transactionBit.data, size, cudaMemcpyHostToDevice); 
    }

    ~Apriori () {
        releaseItemsBit(&transactionBit);
        cudaFree(cuda_transaction_buffer);
    }
    ...
    void generateKFrequentItems (KFrequentItems &ret, ItemsBit *ptr) {
        Bit *cuda_kitems_buffer; 
        int size = ptr->size * transactionBit.unit_len * sizeof(Bit);
        cudaMalloc((void **)&cuda_kitems_buffer, size);
        cudaMemcpy(cuda_kitems_buffer, ptr->data, size, cudaMemcpyHostToDevice); 

        int *result = new int [ptr->size];
        getSupport(cuda_transaction_buffer, transactionBit.size,
                   cuda_kitems_buffer, ptr->size, ptr->unit_len, result);
        cudaFree(cuda_kitems_buffer);

        for (int i = 0; i < ptr->size; i++) {
            ret.push_back({kItemSets[i], (double)result[i] / transactionBit.size * 100.0});
        }
        delete [] result;
