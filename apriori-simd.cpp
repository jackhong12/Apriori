...
#define TransactionAlignedSize 4    // each AVX2 instructions contain 4 long type data
#define TransactionAlignedMask (TransactionAlignedSize - 1)
...
void initTransactionsBit (ItemsBit *ptr, ItemSets &isets) {
    ptr->size = isets.size();
    ptr->bit_len = bit_map_table_size;
    ptr->unit_len = (ptr->bit_len + BitMask) >> BitShift;
    int aligned_size = (ptr->size + TransactionAlignedMask) & ~TransactionAlignedMask;

    ptr->data = new Bit [aligned_size * ptr->unit_len];

    for (int i = 0; i < isets.size(); i++) {
        int start = (i & ~TransactionAlignedMask) * ptr->unit_len;
        int rank = i & TransactionAlignedMask;
        for (int j = 0; j < ptr->unit_len; j++)
            ptr->data[start + j * TransactionAlignedSize + rank] = 0;

        for (auto item: isets[i]) {
            int pos = bit_map_table[item];
            int offset = pos >> BitShift;
            int shift = pos & BitMask;
            ptr->data[start + offset * TransactionAlignedSize + rank] |= (Bit)1 << shift;
        }
    }
}
...
public:
    Apriori (ItemSets _transactions, long double _minSupport) {
        ...
        initTransactionsBit(&transactionBit, transactions);
    }
    ...
    inline long double getSupportSIMD (Bit *items) {
        __m256i avx_mask = _mm256_set1_epi64x(-1);
        __m256i avx_zero = _mm256_set1_epi64x(0);
        __m256i avx_count = _mm256_set1_epi64x(0);
        for (int i = 0; i < transactionBit.size; i += 4) {
            int start = (i & ~TransactionAlignedMask) * transactionBit.unit_len;

            Bit *tmp = &transactionBit.data[start];
            __m256i avx_result = _mm256_set1_epi64x(0);
            for (int j = 0; j < transactionBit.unit_len; j++) {
                __m256i avx_item = _mm256_set1_epi64x(items[j]);
                __m256i avx_trans = _mm256_maskload_epi64(tmp, avx_mask);

                __m256i avx_tmp = _mm256_and_si256(avx_item, avx_trans);
                avx_tmp = _mm256_xor_si256(avx_tmp, avx_item);
                avx_result = _mm256_or_si256(avx_tmp, avx_result);
                tmp += TransactionAlignedSize;
            }
            avx_result = _mm256_cmpeq_epi64(avx_result, avx_zero);
            avx_result = _mm256_srli_epi64(avx_result, 63);
            avx_count = _mm256_add_epi64(avx_count, avx_result);
        }

        long long int results[4];
        int ret = 0;
        _mm256_maskstore_epi64(results, avx_mask, avx_count);
        for (int i = 0; i < 4; i++)
            ret += results[i];
